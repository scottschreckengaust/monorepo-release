...
@cli.command('sync-init-versions')
@click.option('--directory', type=click.Path(exists=True, path_type=Path), default=Path.cwd())
def sync_init_versions(directory: Path) -> int:
    """Synchronize __init__.py versions with pyproject.toml versions."""
    path = directory.resolve(strict=True)
    
    # Find all PyPI packages
    for pyproject_path in path.glob('*/pyproject.toml'):
        package_dir = pyproject_path.parent
        package = PyPiPackage(package_dir)
        
        logging.info(package_dir)
        try:
            # Get version from pyproject.toml
            current_version = package.package_version()
            ...
            if package_name.startswith('awslabs.'):
                module_name = package_name[8:].replace('-', '_')  # Remove 'awslabs.' prefix and convert hyphens
                init_file = package_dir / 'awslabs' / module_name / '__init__.py'
                ...
                if init_file.exists():
                    ...
                    new_version_line = f"__version__ = '{current_version}'"
                    ...
                    if re.search(version_pattern, content):
                        ...
                        click.echo(f"Updated {init_file}: __version__ = '{current_version}'")
                    else:
                        click.echo(f"Warning: No __version__ found in {init_file}")
                else:
                    click.echo(f"Warning: {init_file} not found for package {package_name}")
            else:
                click.echo(f"Warning: Package {package_name} doesn't follow awslabs.* naming convention")
                
        except Exception as e:
            click.echo(f"Error processing {package_dir}: {e}")
